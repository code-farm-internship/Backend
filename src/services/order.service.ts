...
import { changeOrderStatusSchema, createOrderCODSchema, createOrderOnlineSchema, updateOrderSchema } from '@/validations/order/orderSchema';
...
export const updateOrder = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (order.couponId) {
    ...
    if (coupon) {
      const { discount, final } = calculateDiscount(coupon, totalAmount, shippingFee !== undefined ? shippingFee : order.shippingFee);
