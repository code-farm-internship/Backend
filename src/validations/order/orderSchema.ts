...
export const orderItemSchema = Joi.object({
    productVariantId: Joi.string().hex().length(24).trim().required().messages({
    ...
    productId: Joi.string().hex().length(24).trim().required().messages({
...
export const shippingAddressSchema = Joi.object({
    detailAddress: Joi.string().min(1).max(200).trim().required().messages({
    ...
    province: Joi.string().min(1).max(100).trim().required().messages({
    ...
    district: Joi.string().min(1).max(100).trim().required().messages({
    ...
    ward: Joi.string().min(1).max(100).trim().required().messages({
...
export const customerInfoSchema = Joi.object({
    username: Joi.string().min(1).max(50).trim().required().messages({
...
export const receiverInfoSchema = Joi.object({
    recevierName: Joi.string().min(1).max(50).trim().required().messages({
        'string.empty': 'Tên người dùng không được để trống',
        'any.required': 'Tên người dùng là bắt buộc',
        'string.base': 'Tên người dùng phải là chuỗi',
        'string.min': 'Tên người dùng phải lớn hơn hoặc bằng 1 ký tự',
    ...
    recevierPhoneNumber: Joi.string()
        .pattern(/^[0-9]{10}$/)
        .required()
        .messages({
            'string.empty': 'Số điện thoại không được để trống',
...
export const createOrderCODSchema = Joi.object({
    ...
    receiverInfo: receiverInfoSchema.optional().messages({
    ...
    couponCode: Joi.string().min(1).max(50).trim().optional().messages({
    ...
    userNote: Joi.string().min(1).max(500).trim().optional().messages({
...
export const createOrderOnlineSchema = Joi.object({
    ...
    paymentDetails: Joi.object({
        method: Joi.string().min(1).max(50).trim().required().messages({
        ...
        transactionId: Joi.string().min(1).max(100).trim().optional().messages({
...
export const changeOrderStatusSchema = Joi.object({
    ...
    adminNote: Joi.string().min(1).max(500).trim().optional().messages({
